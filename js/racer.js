...
(function () {
  ...
  function setupSearch() {
    ...
    input.addEventListener("change", function () {
      ...
      // 検索確定時はインデックス対象の静的ページへ遷移 (SEO上の正規URL)
      if (id && racers[id]) { location.href = "racer/" + id + ".html"; }
    });
  }
  ...
  function findByQuery(q) {
    q = (q || "").trim();
    if (!q) { return null; }
    if (racers[q]) { return q; }
    var hit = null;
    Object.keys(racers).some(function (k) {
      if (racers[k].racer_name === q || racers[k].racer_name.indexOf(q) === 0) {
        hit = k; return true;
      }
      return false;
    });
  ...
  function route() {
    // 構造化データの SearchAction (?q=) に対応: 一致すれば正規の静的ページへ
    var params = new URLSearchParams(location.search);
    var qid = findByQuery(params.get("q"));
    if (qid) { location.replace("racer/" + qid + ".html"); return; }
